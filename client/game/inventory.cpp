...
namespace Client {
	std::string Inventory::GetPath() {
		return std::format("{}\\players\\inventory.json", g_Pointers->m_Dvar_GetStringSafe("fs_homepath"));
	}
	...
	void Inventory::Save() {
		...
		std::ofstream(GetPath()) << inventoryJson;
	}
	...
	void Inventory::Load() {
		...
		if (std::filesystem::exists(path)) {
			...
			LOG("Inventory", WARN, "Attempted to load inventory from \"{}\" but the file does not exist.", path.c_str());
		}
	}
}
