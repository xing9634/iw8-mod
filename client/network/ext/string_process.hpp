...
namespace Client::Network::Ext {
	...
	inline std::wstring URLEncoding(const std::wstring& keyword, bool convertToUTF8 = true) {
		...
		if (convertToUTF8) {
			iLength = ::WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), keyword.length(), NULL, 0, NULL, NULL);
			if (iLength <= 0) {
				return L"";
			}
			...
			iLength = ::WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), keyword.length(), szKeyword, iLength, NULL, NULL);
		}
		else {
	...
	inline std::wstring GetHost(const std::wstring& url) {
		...
		std::wstring urlWithoutProtocol = ReplaceString(LowerString(url), L"http://", L"");
		unsigned int index = urlWithoutProtocol.find(L"/");

		if (index == std::wstring::npos) {
			index = urlWithoutProtocol.find(L"\\");
		}
		...
		return urlWithoutProtocol.substr(0, index);
	}
